...
public class Compte {
	...
	public Compte(int solde) throws BadParametersException {
		if (solde >= 0)
			...
			throw new BadParametersException(solde
					+ " n'est pas un solde valide dans compte(..) ");
	...
	/**
	 * Renvoie le nombre de jetons disponible sur le compte 
	 * 				@return 
	 * 						le solde du compte
	 */
	 ...
	 * 				est levée si le nouveau montant est <0
	 */
	public void setSolde(long solde) throws BadParametersException {
		if (solde >= 0)
			...
			throw new BadParametersException(solde
					+ " n'est pas un solde valide dans setSolde() ");
	}
	...
	public void debiterCompte(long montant) throws BadParametersException,
		...
		if (montant <= this.solde && montant >= 0) {
			...
			throw new BadParametersException(montant
					+ " est inferieur au solde " + this.solde
					+ "ou n'est pas valide dans debiter compte");
		} else {
	...
	public void crediterCompte(long montant) throws BadParametersException {
		if (montant >= 0) {
			...
			throw new BadParametersException(montant
					+ " est un montant pas valide dans crediter(..) ");
